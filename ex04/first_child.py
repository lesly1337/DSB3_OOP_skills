    ...
    if (len(sys.argv) == 2):
        ...
        for index, line in enumerate(list_of_lines):
            ...
            if ( index != len(list_of_lines) - 1 ):
                print(", ", end = "")
        print("]") 
        print(f"{calc.counts()[0]} {calc.counts()[1]}")
        print(f"{calc.fractions(calc.counts())[0]:.{3}f} {calc.fractions(calc.counts())[1]:.{3}f}")
        analyt = Analytics(list_of_lines)
        print(', '.join([f"[{item}]" for item in analyt.predict_random(3)]))#использовал list comprehension
